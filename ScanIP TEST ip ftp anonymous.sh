#!/bin/bash
	printf '\033[8;45;94t'		# will resize the window.
...
echo Specific softwares variables, you can change theses variables.
	echo
	automatic=0
	echo automatic=$automatic , 0 or 1 , 0 deactivated , 1 activated , default 0
	noquit=0
	echo noquit=$noquit , 0 or 1 , 0 deactivated , 1 activated , default 1
	debug=0
	echo debug=$debug , 0 or 1 , 0 deactivated , 1 activated , default 0
	echo

## Software name, what is this, version, informations.
	echo "Software name: SCANNER IP"
	echo
	echo "Scan ISP netword for responding ip and test open port."
	echo
	...
	echo 2025-02-17_Monday_19:41:12
	globalversion=2025-02-17_Monday_19:41:12
	echo
...
##-------------------------=========== SEPARATOR =============-------------------------
	...
	debug() {
		echo
		echo "Debug data : debug=$debug error=$error part=$part noquit=$noquit automatic=$automatic"
		echo "Debug data : random=$random random2=$random2 primeerror=$primeerror id=$id"
		echo
		}
...
##-------------------------=========== SEPARATOR =============-------------------------
	...
	printf '\033[8;20;94t'				# will resize the window.
...
#specificip=192.168.0.25
	
specificip=$(zenity --entry --title="ScannerIP" --text="Enter an ip to test (eg: 192.168.0.1) DO NOT PUT THE PORT!" --entry-text "192.168.0.1")
...
## The code program.
	...
	functiontest() {
		timeout 0.3 bash -c "</dev/tcp/$specificip/$port" 2> /dev/null
		active=$(echo $?)
		#echo $active
		if [ "$active" -eq "0" ]; then
			echo "${green}█████████ $specificip:$port : OPEN $name Port $port ${reset}"
			echo
			openport=1
			echo Test anonymous connect and quit.
			#if echo -e "user anonymous\nbye" | ftp -n $specificip:$port 2>/dev/null; then				## no timeout but working
			if timeout 3s bash -c 'echo -e "user anonymous\nbye" | ftp -n $specificip:$port' 2>/dev/null; then	## with
					echo "${green}█████████ Connected port $port successfully.${reset}"
					connect=1
				else
					echo "${yellow}█████████ Failed port $port to connect as anonymous.${reset}"
					connect=0
				fi
				echo
			fi
		if [ "$active" -ge "1" ]; then
			echo "${blue}█████████ $specificip:$port : CLOSED $name Port $port ${reset}"
			echo
			fi
		
		if [ "$connect" -eq "1" ]; then
			echo Log-in
			echo "${green}█████████ Connect : ftp ftp://anonymous:anonymous@$specificip:$port ${reset}"
			ftp ftp://anonymous:anonymous@$specificip:$port
			fi
		}
		
		#ftp ftp://anonymous:anonymous@192.168.0.25:21
...
echo "Software lead out."
	...
	echo "	Debug data : debug=$debug error=$error part=$part noquit=$noquit random=$random random2=$random2 automatic=$automatic primeerror=$primeerror id=$id" >>/dev/shm/logs.txt
