...
#specificip=8.7.207.213
	
specificip=$(zenity --entry --title="ScannerIP" --text="Enter an ip to test (eg: 192.168.0.1)" --entry-text "192.168.0.1")

					ping -W 0.2 -c 1 "$specificip" &> /dev/null
					active=$(echo $?)
					if [ "$active" -eq "0" ]; then
						echo "${blue}██████████ $specificip : RESPONDING IP : TEST PORT ${reset}"
					else
						echo "${red}██████████ $specificip : NOT RESPONDING. ${reset}"
					fi
			...
			functionscanmoreports() {
				...
				#echo $active
				if [ "$active" -eq "0" ]; then
					echo "${green}██████████ $specificip : OPEN $name Port $port : ACTIVE ██████████${reset}"
					#echo "## $loop1.$loop2.$loop3.$loop4:$port OPEN $name Port $port" >> "/$HOME/Documents/ScanLOGS/ScanLOGS-$ipname.txt"
					else
						echo Specific ip $specificip $port closed.
					fi
			}	## end function
			...
			## Add extra port, remove # add number and name 
			#port=45764
			#name='PortChecker'
			#functionscanmoreports
